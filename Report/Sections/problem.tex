...
There are a number of models to show relation among different wave dynamics. The first question we faced is that what kind of wave we should simulate in our model. As our interest is on the near shore zone which have a relatively shallow water depth and small amplitude wave (mainly gravity wave), we chose linear wave to fit our model. Based on the basic assumptions such as, incompressible, inviscid and small amplitude, this is a reasonable choice. The shallow water model is the most common way to start modeling this problem with linear wave. This is using the shallow water model which is constructed by continuity equation and simplified Navier-Stokes equation of momentum \cite{dean1991water}. This method is mentioned in 2015. However, we might not go further with it as we did not have any data related to the velocity distribution under water. So we considered to use the energy flux method which is defined as
...
\end{equation} 
In this model \cite{Alex2007}  ${E\,C_{g}}$ represents the wave energy flux. In the energy flux ${E}$ is the energy per unit area \cite{Alex2007} which is defined as
...
The right hand side ${\delta}$ in the model equation (\ref{eqn:forward}) provides local mean rate of energy dissipation\cite{Alex2007}.  
...
In this expression ${\sigma=\frac{2\pi}{T}}$ \cite{dean1991water} is the angular frequency.  
